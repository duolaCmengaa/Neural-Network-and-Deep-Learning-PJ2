...
def train(model, optimizer, criterion, train_loader, val_loader, scheduler=None, epochs_n=20, duration=30):
    ...
    for epoch in tqdm(range(epochs_n), unit='epoch', ncols=80):
...
if __name__ == '__main__':
    ...
    for X, y in train_loader:
        try:
            ...
            print("Wrong! Check the DataLoader.")
            print(e)
        else:
    ...
    epochs = 20
    learning_rates = [0.15, 0.125, 0.1, 0.075, 0.05]
    ...
    for lr in learning_rates:
        print(f'Training VGG-A without BatchNorm, learning rate: {lr}')
        model_vgg = VGG_A()
    ...
    # Plot Loss Landscape
    fig, ax = plt.subplots(figsize=(9, 6), dpi=850)
    ...
    plt.style.use('ggplot')
    fig, axs = plt.subplots(1, 3, figsize=(27, 6), dpi=850)
