...
import numpy as np
from utils.mynn import setup_logging, count_parameters, visualize_metrics
...
def build_model(model_name):
    if model_name == 'ResNet18':
        ...
        return ResNet50() 
    elif model_name == 'ResNet18_with_filtermul':
        return ResNet18_with_filtermul()
    elif model_name == 'ResNet18_with_dropout':
        return ResNet18_with_dropout()
    else:
        raise ValueError(f'Unknown the model name: {model_name}')
...
def train_model(model, trainloader, criterion, optimizer, device, epoch, total_epochs, metrics):
    ...
    correct = 0
    total = 0
    pbar = tqdm(trainloader, desc=f'Training Epoch {epoch+1}/{total_epochs}', ncols=100, leave=True)
    for batch_idx, (inputs, targets) in enumerate(pbar):
        ...
        pbar.set_postfix({'Train_loss': f'{train_loss/(batch_idx+1):.3f}',
                         'Train_accuracy': f'{100.*correct/total:.3f}%'})
    logging.info(pbar)
...
def test_model(model, testloader, criterion, device, epoch, total_epochs, metrics):
    ...
    correct = 0
    total = 0
    pbar = tqdm(testloader, desc=f'Testing Epoch {epoch+1}/{total_epochs}', ncols=100)
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(pbar):
            inputs, targets = inputs.to(device), targets.to(device)
            outputs = model(inputs)
            ...
            pbar.set_postfix(
                {'Test_loss': f'{test_loss/(batch_idx+1):.3f}', 'Te_acc': f'{100.*correct/total:.3f}%'})
        logging.info(pbar)
...
def main():
    ...
    print(f'Now building and training model: {model_name} ..')
    # Setup logger
    ...
    logging.info(f'The total parameters: {total_params}')
    
    if args.resume:
        # Load checkpoint.
        print(f'Loading {model_name} from checkpoint..')
        checkpoint_path = f'./checkpoints/{model_name}_ckpt.pth'
        assert os.path.isfile(checkpoint_path), f'Error: there is no checkpoint in {model_name}'
    ...
    metrics = {'train_loss': [], 'test_loss': [], 'train_accuracy': [], 'test_accuracy': [], 'lr': []}
    ...
    logging.info(f'The Total parameters: {total_params}')
    logging.info(f'The Total training time: {total_train_time:.2f}s, Average epoch time: {avg_epoch_time:.2f}s')
